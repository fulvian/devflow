...
check_db_activity() {
    ...
    if [ -f "$db_path" ]; then
        ...
        local recent_reads=$(sqlite3 "$db_path" "SELECT COUNT(*) FROM audit_log WHERE operation IN ('SELECT', 'READ') AND created_at > datetime('now', '-5 seconds')" 2>/dev/null || echo "0")
        local recent_writes=$(sqlite3 "$db_path" "SELECT COUNT(*) FROM audit_log WHERE operation IN ('INSERT', 'UPDATE', 'DELETE') AND created_at > datetime('now', '-5 seconds')" 2>/dev/null || echo "0")
...
get_token_counters() {
    ...
    # Try to read from token state
    if [ -f ".devflow/token-usage-state.json" ]; then
