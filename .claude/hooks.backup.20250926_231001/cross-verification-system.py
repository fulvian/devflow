...
class CrossVerificationHook(PostToolUseHook):
    """Stop/PostToolUse hook that triggers cross-verification"""
    ...
    def validate_input(self) -> bool:
        ...
        if hook_event == 'Stop':
            ...
            missing = [f for f in required_fields if f not in self.input_data]
            if missing:
                self.logger.error(f"PostToolUse hook missing fields: {missing}")
                return False
            return True
    ...
    def execute_logic(self):
        ...
        if hook_event == 'Stop':
            # Handle Stop event - generate session summary
    ...
    def _handle_stop_event(self):
        """Handle Stop event - generate session verification summary"""
        try:
            session_id = self.input_data.get('session_id')

            # Get verification stats for current session
            verification_stats = self.verification_system.get_verification_stats(days=1)

            # Log session completion
            self.logger.info(f"Session {session_id} completed - generating verification summary")

            # Add summary to response metadata
            self.response.metadata.update({
                'session_completed': True,
                'session_id': session_id,
                'verification_summary': verification_stats.get('summary', {}),
                'requires_final_review': verification_stats.get('summary', {}).get('failed', 0) > 0
            })

        except Exception as e:
            self.logger.error(f"Error handling stop event: {e}")
            self.response.metadata.update({
                'session_completed': True,
                'error': str(e)
            })
