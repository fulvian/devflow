#!/usr/bin/env python3
"""
Session Start Hook - Context7 Implementation

DevFlow session initialization hook for Claude Code Sessions.
Initializes session context, loads configuration, and prepares development environment.
...
class SessionStartHook(BaseDevFlowHook):
    """Context7-compliant session start hook"""

    def __init__(self):
        super().__init__("SessionStart")

    def validate_input(self) -> bool:
        """Validate SessionStart hook input"""
        # SessionStart doesn't require specific fields, just basic validation
        return self.input_data.get('hook_event_name') == 'SessionStart'

    def execute_logic(self):
        """Execute session start logic with Context7 compliance"""
        # Generate session context using existing logic
        context = generate_session_context()

        # Use Context7 pattern for additional context
        self.response.additional_context = context
        self.response.metadata.update({
            'session_context_generated': True,
            'context_length': len(context),
            'setup_needed': needs_setup if 'needs_setup' in globals() else False,
...
# Context7 as default - legacy compatibility available with --legacy flag
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == '--legacy':
        # Legacy mode - generate context directly
        context = generate_session_context()
        output = {
            "hookSpecificOutput": {
        ...
        # Context7 mode - use unified pattern
        sys.exit(main())