...
export class DualEnhancedSyntheticMCPServer {
  ...
  private async handleDualAutonomousFileOperation(args: {
    ...
    // Enhanced system prompt with storage awareness
    const systemPrompt = `You are a dual-mode autonomous code generation AI that works with both cc-sessions (.md files) and multi-layer (SQLite + Vector) storage systems.
