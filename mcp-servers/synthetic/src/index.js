...
class SyntheticMCPServer {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                switch (name) {
                    ...
                    case 'synthetic_auto':
                        return await this.handleAutonomousTask(args);
                    default:
    ...
    async callSyntheticAPI(model, messages, maxTokens = 4000) {
        if (!SYNTHETIC_API_KEY) {
            throw new Error('SYNTHETIC_API_KEY not configured');
        }
        const response = await axios.post(`${SYNTHETIC_API_URL}/chat/completions`, {
            model,
            messages,
...
Generate the code with proper documentation and structure.`;
        const response = await this.callSyntheticAPI(DEFAULT_CODE_MODEL, [
...
Provide comprehensive reasoning and analysis.`;
        const response = await this.callSyntheticAPI(DEFAULT_REASONING_MODEL, [
            { role: 'system', content: systemPrompt },
            { role: 'user', content: userPrompt },
        ]);
        return {
...
Please provide ${args.analysis_type || 'explanation'} focusing on: ${args.focus || 'general analysis'}`;
        const response = await this.callSyntheticAPI(DEFAULT_CONTEXT_MODEL, [
            { role: 'system', content: systemPrompt },
            { role: 'user', content: userPrompt },
        ]);
        return {
...
- Analysis: ${args.analysis_type || 'explain'}`,
    ...
    async handleAutonomousTask(args) {
...
Provide a brief explanation of your classification.`;
        const classificationResponse = await this.callSyntheticAPI(DEFAULT_CONTEXT_MODEL, [{ role: 'user', content: classificationPrompt }]);
