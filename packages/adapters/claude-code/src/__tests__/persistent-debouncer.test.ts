...
describe('PersistentDebouncer', () => {
  it('persists and resumes pending tasks', async () => {
    ...
    try { rmSync(journal); } catch {}

    const d1 = new PersistentDebouncer<{ msg: string }>({ delayMs: 10, journalPath: journal });
    const fn = vi.fn();
    ...
    const d2 = new PersistentDebouncer<{ msg: string }>({ delayMs: 10, journalPath: journal });
