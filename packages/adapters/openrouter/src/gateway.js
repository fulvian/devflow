...
import { DEFAULT_MODELS, findModelSpec } from './models/model-config.js';
...
export class OpenRouterGateway {
    ...
    constructor(cfg = {}) {
        ...
        if (cfg.timeoutMs !== undefined)
            clientCfg['timeoutMs'] = cfg.timeoutMs;
        if (cfg.maxRetries !== undefined)
            clientCfg['retries'] = cfg.maxRetries;
        this.client = new OpenRouterClient(clientCfg);
    ...
    mergeContext(messages, injected) {
        if (!injected)
            return messages;
        const sys = { role: 'system', content: injected };
        return [sys, ...messages];
    }
    async generate(input) {
        // Rough token estimate: 1 token ~= 4 chars
        const approxTokens = Math.floor(input.messages.reduce((acc, m) => acc + m.content.length, 0) / 4);
        const baseRoute = { description: input.description };
        if (input.title !== undefined)
            baseRoute.title = input.title;
        ...
        for (const modelId of order) {
            ...
            try {
                ...
                if (input.maxTokens !== undefined)
                    reqBase.max_tokens = input.maxTokens;
                if (input.temperature !== undefined)
                    reqBase.temperature = input.temperature;
