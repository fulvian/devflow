...
import { CostMonitor, type CostBudget, type CostAlert } from './cost-monitor.js';
...
export class SmartOpenRouterGateway {
  ...
  private config: SmartGatewayConfig;
   ...
   */
  async smartGenerate(input: GenerateInput & { 
    ...
    try {
      ...
      // Record usage
      if (this.costMonitor) {
        const alerts = await this.costMonitor.recordUsage(
          selectedModel, 
          result.raw.usage?.total_tokens || 0, 
          actualCost
        );
   ...
   */
  private async tryDeepSeekFallback(input: GenerateInput, originalError: Error): Promise<SmartGenerateResult> {
   ...
   */
  private async executeWithModel(model: string, input: GenerateInput): Promise<GenerateResult> {
   ...
   */
  private classifyTaskComplexity(description: string): 'simple' | 'medium' | 'complex' {
   ...
   */
  private getOptimalModel(complexity: 'simple' | 'medium' | 'complex', _estimatedTokens: number): string {
    if (this.costMonitor) {
   ...
   */
  async testModels(): Promise<Record<string, { success: boolean; responseTime: number; error?: string }>> {
    const testModels = [
      'deepseek/deepseek-chat-v3.1:free',
      'deepseek/deepseek-chat-v3.1',
      'gpt-4o-mini'
