export class SyntheticCostTracker {
    records = [];
    monthlyFeeUsd = 20; // $20/month flat fee
    constructor() {
     ...
     */
    getStats(rangeMs = 30 * 24 * 60 * 60 * 1000) {
        const now = Date.now();
     ...
     */
    getAgentBreakdown(rangeMs = 30 * 24 * 60 * 60 * 1000) {
        const now = Date.now();
        const recent = this.records.filter(r => now - r.timestamp <= rangeMs);
     ...
     */
    getPayPerUseSavings(payPerTokenRate = 0.002) {
     ...
     */
    isGoodValue(minTokensForValue = 50000) {
        const stats = this.getStats();
        return {
    ...
    calculateFlatFeePortion(tokens) {
