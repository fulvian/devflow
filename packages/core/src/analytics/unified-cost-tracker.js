export class UnifiedCostTracker {
     ...
     */
    getSummary(platform, rangeMs = 30 * 24 * 60 * 60 * 1000 // Default: 30 days
    ) {
        const now = Date.now();
        const relevantEvents = this.events.filter(e => now - e.timestamp <= rangeMs &&
     ...
     */
    getCostBreakdown(period, count = 7) {
        const periodMs = {
            hour: 60 * 60 * 1000,
            day: 24 * 60 * 60 * 1000,
        ...
        for (let i = 0; i < count; i++) {
            ...
            const periodEvents = this.events.filter(e => e.timestamp >= startTime && e.timestamp < endTime);
            const platforms = {};
            let total = 0;
    ...
    calculateSummaryForEvents(events, platform) {
        if (events.length === 0) {
            return {
                platform,
                totalRequests: 0,
    ...
    getCostForPeriod(startTime, endTime) {
        return this.events
...
//# sourceMappingURL=unified-cost-tracker.js.map