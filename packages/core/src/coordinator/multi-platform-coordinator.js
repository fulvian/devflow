export class MultiPlatformCoordinator {
     ...
     */
    async executeOnPlatform(request, platform, analysis) {
     ...
     */
    async executeOnSynthetic(request, analysis) {
        if (!this.synthetic) {
     ...
     */
    async executeOnOpenRouter(request, analysis) {
        if (!this.openRouter) {
    ...
    getCostStatistics() {
        return {
            synthetic: this.synthetic ? this.synthetic.getCostStats() : null,
    ...
    estimateTokens(description, title) {
        const text = `${title || ''} ${description}`;
        return Math.ceil(text.length / 4); // Rough estimate: 4 chars per token
    }
    estimateCost(platform, tokens) {
    ...
    calculateTotalCosts() {
        const synthetic = this.synthetic ? this.synthetic.getCostStats() : null;
        return {
...
//# sourceMappingURL=multi-platform-coordinator.js.map