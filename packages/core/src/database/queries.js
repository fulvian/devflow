...
export class Queries {
    ...
    createTaskContext(input) {
        ...
        const stmt = this.db.prepare(`INSERT INTO task_contexts (
        ...
        const info = stmt.run(data.title, data.description ?? null, data.priority, data.status ?? null, data.complexityScore ?? null, data.estimatedDurationMinutes ?? null, data.requiredCapabilities ? JSON.stringify(data.requiredCapabilities) : null, data.primaryPlatform ?? null, data.platformRouting ? JSON.stringify(data.platformRouting) : null, JSON.stringify(data.architecturalContext ?? {}), JSON.stringify(data.implementationContext ?? {}), JSON.stringify(data.debuggingContext ?? {}), JSON.stringify(data.maintenanceContext ?? {}), data.ccSessionId ?? null, data.ccTaskFile ?? null, data.branchName ?? null, data.parentTaskId ?? null, data.dependsOn ? JSON.stringify(data.dependsOn) : null);
        const row = this.db.prepare('SELECT id FROM task_contexts WHERE rowid = ?').get(info.lastInsertRowid);
        return row.id;
    }
    getTaskContext(id) {
        const row = this.db.prepare('SELECT * FROM task_contexts WHERE id = ?').get(id);
        if (!row)
            return null;
        return {
            id: row.id,
    ...
    updateTaskContext(id, updates) {
        ...
        if (updates.primaryPlatform !== undefined) {
            fields.push('primary_platform = ?');
            values.push(updates.primaryPlatform);
        }
        if (updates.platformRouting !== undefined) {
            fields.push('platform_routing = ?');
            values.push(JSON.stringify(updates.platformRouting));
        }
        if (updates.completedAt !== undefined) {
            fields.push('completed_at = ?');
            values.push(toISO(updates.completedAt) ?? null);
        }
        if (fields.length === 0)
            return;
    ...
    storeMemoryBlock(input) {
      ...
      INSERT INTO memory_blocks (
        ...
        const info = stmt.run(data.taskId, data.sessionId, data.blockType, data.label, data.content, JSON.stringify(data.metadata ?? {}), data.importanceScore ?? 0.5, JSON.stringify(data.relationships ?? []), data.embeddingModel ?? null);
        const row = this.db.prepare('SELECT id FROM memory_blocks WHERE rowid = ?').get(info.lastInsertRowid);
        return row.id;
    }
    retrieveMemoryBlocks(q) {
        ...
        if (safe.taskId) {
            where.push('task_id = ?');
            params.push(safe.taskId);
        }
        if (safe.blockTypes && safe.blockTypes.length) {
            where.push(`block_type IN (${safe.blockTypes.map(() => '?').join(',')})`);
            params.push(...safe.blockTypes);
        }
        if (safe.platforms && safe.platforms.length) {
            where.push(`json_extract(metadata, '$.platform') IN (${safe.platforms.map(() => '?').join(',')})`);
            params.push(...safe.platforms);
        }
        if (safe.dateRange) {
            where.push('created_at BETWEEN ? AND ?');
            params.push(toISO(safe.dateRange.start), toISO(safe.dateRange.end));
        }
        if (typeof safe.importanceThreshold === 'number') {
            where.push('importance_score >= ?');
            params.push(safe.importanceThreshold);
        }
    ...
    startSession(session) {
      ...
      INSERT INTO coordination_sessions (
        ...
        const info = stmt.run(data.taskId, data.platform, data.sessionType ?? 'development', data.tokensUsed ?? 0, data.apiCalls ?? 0, data.estimatedCostUsd ?? 0, data.modelUsed ?? null, data.contextSizeStart ?? null, data.contextSizeEnd ?? null, data.compactionEvents ?? 0, data.handoffFromSession ?? null, data.handoffToSession ?? null, data.handoffContext ? JSON.stringify(data.handoffContext) : null, data.handoffSuccess ?? null, data.userSatisfaction ?? null, data.taskProgressDelta ?? 0, data.errorsEncountered ?? 0, data.metadata ? JSON.stringify(data.metadata) : '{}');
        const row = this.db.prepare('SELECT id FROM coordination_sessions WHERE rowid = ?').get(info.lastInsertRowid);
        return row.id;
    }
    endSession(sessionId, metrics) {
        this.db.prepare(`
      ...
      WHERE id = ?
    `).run(metrics.tokensUsed, metrics.apiCalls, metrics.estimatedCostUsd, metrics.contextSizeEnd ?? null, metrics.compactionEvents ?? null, metrics.userSatisfaction ?? null, metrics.taskProgressDelta ?? null, metrics.errorsEncountered ?? null, metrics.metadata ? JSON.stringify(metrics.metadata) : null, sessionId);
    }
}
