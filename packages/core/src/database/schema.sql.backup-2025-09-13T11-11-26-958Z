-- DevFlow Foundation Database Schema
-- SQLite with JSON1, FTS5, and Vector extensions
-- Version: 1.0.0
...
CREATE TRIGGER update_memory_access 
...
CREATE TRIGGER tasks_fts_insert AFTER INSERT ON task_contexts BEGIN
    INSERT INTO tasks_fts(rowid, title, description) 
    VALUES (NEW.rowid, NEW.title, COALESCE(NEW.description, ''));
END;

CREATE TRIGGER tasks_fts_delete AFTER DELETE ON task_contexts BEGIN
    INSERT INTO tasks_fts(tasks_fts, rowid, title, description) 
    VALUES ('delete', OLD.rowid, OLD.title, COALESCE(OLD.description, ''));
END;

CREATE TRIGGER tasks_fts_update AFTER UPDATE ON task_contexts BEGIN
    INSERT INTO tasks_fts(tasks_fts, rowid, title, description) 
    VALUES ('delete', OLD.rowid, OLD.title, COALESCE(OLD.description, ''));
    INSERT INTO tasks_fts(rowid, title, description) 
    VALUES (NEW.rowid, NEW.title, COALESCE(NEW.description, ''));
...
-- Active tasks with latest session info
CREATE VIEW active_tasks_with_sessions AS
SELECT 
    tc.id,
    tc.title,
    tc.priority,
...
CREATE VIEW platform_efficiency_summary AS
SELECT 
    platform,
...
INSERT INTO platform_performance (platform, capability_domain, task_type, confidence_score, total_tasks, measurement_period_start) VALUES
