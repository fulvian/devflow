...
export class SemanticSearchService {
    ...
    searchService;
    vectorService;
    constructor(_db, searchService, vectorService) {
        this._db = _db;
    ...
    async hybridSearch(query, options = {}) {
        const { mode = 'hybrid', weights = { keyword: 0.5, semantic: 0.5 }, fusionMethod = 'weighted', threshold = 0.0, maxResults = 20, blockTypes, taskIds } = options;
     ...
     */
    normalizeSimilarityScores(results) {
        if (results.length === 0)
            return new Map();
     ...
     */
    calculateHybridScore(keywordScore, semanticScore, weights, method) {
     ...
     */
    extractKeywordMatches(blockId, keywordResults) {
     ...
     */
    extractSemanticContext(text, maxLength) {
     ...
     */
    generateRankingExplanation(keywordScore, semanticScore, hybridScore, method) {
