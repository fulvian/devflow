...
exports.TaskHierarchyService = exports.DatabaseError = exports.TaskHierarchyError = exports.TaskNotFoundError = exports.Platform = exports.TaskPriority = exports.TaskStatus = void 0;
...
(function (Platform) {
    ...
    Platform["OPENAI_CODEX"] = "openai_codex";
    Platform["GEMINI_CLI"] = "gemini_cli";
...
class TaskHierarchyError extends Error {
    constructor(message) {
        super(message);
        this.name = 'TaskHierarchyError';
    }
}
exports.TaskHierarchyError = TaskHierarchyError;
...
class TaskHierarchyService {
    ...
    async createTask(input) {
        ...
        try {
            ...
            if (input.parentTaskId) {
                const parentExists = await db.get('SELECT 1 FROM task_contexts WHERE id = ?', input.parentTaskId);
                if (!parentExists) {
                    throw new TaskHierarchyError(`Parent task ${input.parentTaskId} does not exist`);
                }
            }
            // Insert new task - let SQLite generate ID
            const result = await db.run(`INSERT INTO task_contexts 
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, input.title, input.description || null, priority, status, input.complexityScore || null, input.estimatedDurationMinutes || null, input.requiredCapabilities ? JSON.stringify(input.requiredCapabilities) : null, input.primaryPlatform || null, input.platformRouting ? JSON.stringify(input.platformRouting) : null, JSON.stringify({}), // architectural_context
            JSON.stringify({}), // implementation_context
            JSON.stringify({}), // debugging_context
            JSON.stringify({}), // maintenance_context
            input.ccSessionId || null, input.ccTaskFile || null, input.branchName || null, input.parentTaskId || null, input.dependsOn ? JSON.stringify(input.dependsOn) : null);
            // Get the inserted task using rowid
            const task = await db.get(`SELECT * FROM task_contexts WHERE rowid = ?`, result.lastID);
            if (!task) {
    ...
    async getTaskById(id) {
        ...
        try {
            const row = await this.db.get(`SELECT * FROM task_contexts WHERE id = ?`, id);
            if (!row) {
                return null;
            }
    ...
    async updateTask(id, input) {
        ...
        try {
            ...
            // Execute update
            await db.run(`UPDATE task_contexts SET ${updates.join(', ')} WHERE id = ?`, ...values);
            // Return updated task
    ...
    async deleteTask(id) {
        ...
        try {
            ...
            const childTasks = await db.get('SELECT 1 FROM task_contexts WHERE parent_task_id = ? LIMIT 1', id);
            if (childTasks) {
    ...
    async getChildTasks(parentId) {
        ...
        try {
            ...
            const rows = await this.db.all(`SELECT * FROM task_contexts WHERE parent_task_id = ? ORDER BY created_at ASC`, parentId);
    ...
    async getRootTasks() {
        ...
        try {
            const rows = await this.db.all(`SELECT * FROM task_contexts WHERE parent_task_id IS NULL ORDER BY created_at ASC`);
            return rows.map(row => this.mapRowToTaskContext(row));
        }
        catch (error) {
     ...
     */
    async validateTemporalConsistency(taskId) {
        if (!this.db) {
            throw new DatabaseError('Database not initialized');
        }
        try {
...
exports.TaskHierarchyService = TaskHierarchyService;
