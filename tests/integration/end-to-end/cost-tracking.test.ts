...
import type { CostAnalysis, ModelUsageStats, OptimizationReport } from '../../../packages/shared/src/types.js';

describe('Cost Tracking & Optimization', () => {
  let devFlow: DevFlowCore;
  let openRouter: OpenRouterGateway;
  ...
  describe('Cost Optimization Analysis', () => {
    it('should generate optimization recommendations', async () => {
      ...
      const hasModelOptimization = optimizationReport.recommendations.some(
        rec => rec.type === 'model_optimization'
      );
      const hasUsageOptimization = optimizationReport.recommendations.some(
        rec => rec.type === 'usage_optimization'
      );
