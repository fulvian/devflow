...
export class PerformanceBenchmark {
     ...
     */
    async benchmarkContextOperations(iterations) {
        const results = [];
     ...
     */
    async benchmarkMemoryOperations(iterations) {
        const results = [];
        const storedIds = [];
        // Store phase
        for (let i = 0; i < iterations; i++) {
     ...
     */
    async benchmarkEndToEndWorkflow(iterations) {
        const results = [];
        for (let i = 0; i < iterations; i++) {
     ...
     */
    async benchmarkConcurrentOperations(concurrency) {
        ...
        const sessionPromises = Array(concurrency.sessions).fill(0).map(async (_, sessionIndex) => {
            ...
            for (let opIndex = 0; opIndex < concurrency.operationsPerSession; opIndex++) {
                operations.push(this.devFlow.memory.store({
     ...
     */
    generateReport(benchmarkResults, targets) {
    ...
    countPassedTests(results, targets) {
        let passed = 0;
